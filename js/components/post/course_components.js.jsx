...
class CourseCode extends React.Component {
  ...
  componentWillMount() {
    this.setState({
      selected:
        localStorage.getItem(this.getIdName()) === "active" ||
  ...
  getCategory() {
    ...
    if (this.props.courseIDs[0] === "mat135") {
      ...
      return (
        <p className="code">
          (
          <span id="mat135" className="courseName" onClick={this.openModal}>
            MAT135H
          </span>
          <span> and </span>
          <span id="mat136" className="courseName" onClick={this.openModal}>
            MAT136H
          </span>
          )<span> or </span>
          <span id="mat137" className="courseName" onClick={this.openModal}>
            MAT137Y
          </span>
          <span> or </span>
          <span id="mat157" className="courseName" onClick={this.openModal}>
            MAT157Y
          </span>
        </p>
      )
...
export class MultipleCourseCode extends React.Component {
  ...
  checkIfCompleted() {
    this.setState({
      completed: this.state.completedTextBoxes === this.props.textBoxNumber,
    })
  }
  ...
  render() {
    ...
    return (
      <div id={courseID} className={classes}>
        ...
        <div
          id={"spec" + this.props.courseID.substring(5, this.props.courseID.length)}
          className="more-info"
        >
          <p className="full_name">
            {Array.apply(0, Array(this.props.textBoxNumber)).map((x, i) => {
              if (
                this.isValidExtraCourse(this.state.textboxValues[i].substring(0, 6))
              ) {
        ...
        <ul className="year_course_list">
          ...
          <li>
            {props.titles.map((title, i) => {
              return (
                <MultipleCourseCode
                  courseID={postType + "_category_" + (i + 1)}
          ...
          <li>
            {(() => {
              if (hasInquiryCategory && yearName == "Later Years") {
                return (
                  <InquiryCategory
                    courseID={postType + "_inq"}
                    course={getInquiryCourse()}
