...
module RequirementTests.ModifierTests (test_modifier) where
...
simpleModAndInputs =
    ...
    ,
        ( [Department "CSC", Level "300+", Requirement (Raw "some raw text")]
        , show globalFces ++ " CSC FCEs at the 300+ level from some raw text"
        )
...
modandModOrInputs =
    [ ([ModOr [Level "300", Level "400"], Department "CSC"], show globalFces ++ " CSC FCEs at the 300/400 level")
    ,
        ( [Level "300+", ModOr [Department "CSC", Department "BCB", Department "Statistics"]]
        , show globalFces ++ " CSC/BCB/Statistics FCEs at the 300+ level"
        )
    ,
        ( [ModOr [Level "300", Level "400"], ModOr [Department "CSC", Department "BCB"]]
        , show globalFces ++ " CSC/BCB FCEs at the 300/400 level"
        )
    ,
        ( [ModOr [Level "300", Level "400"], ModOr [Department "CSC", Department "BCB"], Requirement (Raw "some raw text")]
        , show globalFces ++ " CSC/BCB FCEs at the 300/400 level from some raw text"
        )
    ]
...
test_modifier :: TestTree
test_modifier = testGroup "ReqParser tests" [concatModOrTests, simpleModAndTests, modandModOrTests]
