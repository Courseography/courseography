...
createTest :: Either Parsec String Parser Req -> (String, Req) -> Test
...
reqParserTest1 = let courseReq = Parsec.parse courseParser "" " csc148h1 "
                 TestCase(assertEqual "for (gradeParser \" csc148h1 \")," (Right $ J "csc148h1") courseReq)

reqParserTest2 :: Test
reqParserTest2 = let courseReq = Parsec.parse orParser "" " csc148h1 "
                 TestCase(assertEqual "for (orParser \" csc148h1 \")," (Right $ OR[J "csc148h1"]) courseReq)

reqParserTest3 :: Test
reqParserTest3 = let courseReq = Parsec.parse andParser "" " csc148h1,csc165h1 "
                 TestCase(assertEqual "for (andParser \" csc148h1,csc165h1 \")," (Right $ AND[J "csc148h1",J "csc165h1"]) courseReq)

reqParserTest4 :: Test
reqParserTest4 = let courseReq = Parsec.parse andParser "" " csc108h1/csc148h1,csc165h1 "
                 TestCase(assertEqual "for (andParser \" csc108h1,csc148h1,csc165h1 \")," (Right $ AND[OR[J "csc108h1",J "csc148h1"],J "csc165h1"]) courseReq)

-- fcesTests
fcesInputs:: [(String, String)]
fcesInputs = [("5.0 FCEs", "5.0"),("1 FCEs", "1"),("5.0 asdf", "2.5"),("5.0 FCEs", "5.0"),("80", "80"),("6.0", "6.0"),("-A", "A")]
fcesTests :: Test
fcesTests = TestList $ (map (\inputToTest -> createTest fcesParser) fcesInputs)

-- gradesTests
gradesInputs:: [(String, String)]
gradesInputs = [("5.0 FCEs", "5.0"),("1 FCEs", "1"),("5.0 asdf", "2.5"),("5.0 FCEs", "5.0"),("80", "80"),("6.0", "6.0"),("-A", "A")]
gradesTests :: Test
gradesTests = TestList $ (map (\inputToTest -> createTest gradesParser) gradesInputs)
...
coBefTests :: Test
coBefTests = TestList $ (map (\inputToTest -> createTest coBefParser) coBefInputs)
...
coAftTests :: Test
coAftTests = TTestList $ (map (\inputToTest -> createTest coAftParser) coAftInputs)

-- singleParserTests
singleInputs :: [(String, Req)]
singleInputs = [("csc108h1", J "csc108h1"), ("csc165Y1", J "csc165Y1"), ("CSC207H1", J "CSC207H1")]
singleTests :: Test
singleTests = TestList $ (map (\inputToTest -> createTest singleParser) singleInputs)
...
orTests :: Test
orTests = TestList $ (map (\inputToTest -> createTest orParser) orInputs)
...
andTests :: Test
andTests = TestList $ (map (\inputToTest -> createTest andParser) andInputs)
...
fromTests :: Test
fromTests = TestList $ (map (\inputToTest -> createTest fromParser) fromInputs))
