...
module Svg.Parser (parsePrebuiltSvgs, parseDynamicSvg) where
...
import Database.Tables (
    ...
    Path (Path, pathFill),
    Point,
    Shape (Shape, shapeFill, shapePos, shapeType_, shapeWidth),
    Text (Text),
 )
...
performParse ::
    ...
    T.Text ->
    -- | The filename of the file that will be parsed.
    String ->
    SqlPersistM ()
...
performParseFromMemory ::
    -- | The title of the graph
    T.Text ->
    -- | Filename of the SVG to parse
    T.Text ->
    -- | True if SVG is dynamically generated
    Bool ->
    SqlPersistM ()
performParseFromMemory graphName graphSvg isDynamic = do
  ...
  where
    (graphWidth, graphHeight) = parseSizeFromSvg graphSvg
...
parseGraph key tags =
  ...
  where
    ...
    removeRedundant shapes =
        filter
            ( not . \s ->
                shapePos s `elem` map shapePos shapes
                    && isEdge s
                    && elem (shapeType_ s) [Node, Hybrid]
            )
            shapes
...
parseTextHelper ::
    -- | The Text's corresponding graph identifier.
    GraphId ->
    [(T.Text, T.Text)] ->
    Matrix ->
    Matrix ->
    ...
    if not $ any (TS.isTagOpenName "tspan") restTags
        then
            ...
            let tspanTags = TS.partitions (TS.isTagOpenName "tspan") (headTag : restTags)
             in concatMap (parseTextHelper key newStyle newTrans globalTrans) tspanTags
  where
...
parseRect ::
    Matrix ->
    -- | The Rect's corresponding graph identifier.
    GraphId ->
    [Tag T.Text] ->
    [Shape]
parseRect _ _ [] = []
  ...
  where
    ...
    makePoly polyOpenTag =
        let points = map parseCoord (T.splitOn " " $ fromAttrib "points" polyOpenTag)
         ...
         in updateShape (fromAttrib "fill" polyOpenTag) $
                Shape
                    key
...
parsePath globalTrans key tags =
  ...
  where
    ...
    edgeInfo = case splitArr of
        ...
        _ -> ("", "")
      where
        splitArr = T.splitOn "|" (fromAttrib "id" $ safeHead (TS.TagOpen T.empty []) tags)

parsePathHelper ::
    -- | The Path's corresponding graph identifier.
    GraphId ->
    Matrix ->
    Matrix ->
...
parseEllipseHelper ::
    -- | The related graph id.
    GraphId ->
    -- | The translation to apply.
    Matrix ->
    -- | The graph's global transformation.
    Matrix ->
    -- | The open ellipse tag.
    Tag T.Text ->
    -- | The id of the shape.
    T.Text ->
    Shape
...
parseTransform transform =
  ...
  where
    ...
    rotate = do
        ...
        return
            [ [cos angle, -sin angle, xRot * (1 - cos angle) + yRot * sin angle]
...
parsePathD = parseValWithState parser initialState
  where
    ...
    bezier =
        -- Discard the next two points. TODO: Add full support for Bezier curves
        (P.char 'C' <|> P.char 'c')
            >> P.spaces
            >> point
            >> P.spaces
    ...
    moveArg = do
        -- Read a movement argument (can be a point or a double).
        val <- double
        sep <- P.option ' ' $ P.char ','
        if sep == ','
            then do
...
updatePath ::
    -- | The fill that may be added to the Path.
    T.Text ->
    Path ->
    Path
updatePath fill p =
...
updateShape ::
    -- | The fill that may be added to the Shape.
    T.Text ->
    Shape ->
    Shape
updateShape fill r =
    r
        { shapeFill =
            if T.null (shapeFill r) || shapeFill r == "none"
                then fill
                else shapeFill r
        , shapeType_ =
            if fill == "#888888"
