...
import Database.Tables (
    Matrix,
    Path (pathId_, pathIsRegion, pathPoints, pathSource, pathTarget, pathTransform),
    Point,
    Shape (shapeHeight, shapeId_, shapePos, shapeText, shapeTransform, shapeType_, shapeWidth),
...
buildPath ::
    ...
    -- | A path.
    Path ->
    -- | A number to use in the ID of the path.
    Integer ->
    Path
buildPath rects ellipses entity elementId
    | pathIsRegion entity =
        entity
            { pathId_ = pathId_ entity <> "p" <> showt elementId
            , pathSource = ""
            , pathTarget = ""
            }
    | otherwise =
...
buildRect ::
    -- | A list of shapes that may intersect with the given node.
    [Text] ->
    -- | A node.
    Shape ->
    -- | An integer to uniquely identify the shape
    Integer ->
    Shape
...
buildEllipses ::
    -- | A list of Text elements that may or may not intersect
    --   with the given ellipse.
    [Text] ->
    -- | An ellipse.
    Shape ->
    -- | A number to use in the ID of the ellipse.
    Integer ->
    Shape
...
intersects ::
    ...
    -- | The region's height.
    Double ->
    -- | The region's bottom-left coordinate.
    Point ->
    -- | The tolerance for the point being outside the boundary.
    Double ->
