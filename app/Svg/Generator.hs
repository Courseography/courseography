...
module Svg.Generator (buildSVG) where
...
import Database.Tables (
    EntityField (GraphTitle, PathGraph, ShapeGraph, ShapeType_, TextGraph),
    Graph (graphHeight, graphWidth),
    Path (pathFill, pathId_, pathIsRegion, pathPoints, pathSource, pathTarget, pathTransform),
    Shape (shapeFill, shapeHeight, shapeId_, shapePos, shapeText, shapeTransform, shapeType_, shapeWidth),
    Text (textAlign, textFill, textPos, textText, textTransform),
 )
import Svg.Builder
...
buildSVG ::
    -- | The name of the graph that is being built.
    T.Text ->
    -- | A map of courses that holds the course
    ...
    M.Map T.Text T.Text ->
    -- | The file handler that this graph will be
    --   written to.
    Handle ->
...
makeSVGDoc ::
    M.Map T.Text T.Text ->
    -- | A list of the Nodes that will be included
    --   in the graph. This includes both Hybrids and
    --   course nodes.
    [Shape] ->
    -- | A list of the Ellipses that will be included
    --   in the graph.
    [Shape] ->
    -- | A list of the Edges that will be included
    --   in the graph.
    [Path] ->
    -- | A list of the Regions that will be included
    --   in the graph.
    [Path] ->
    -- | A list of the 'Text' elements that will be included
    --   in the graph.
    [Text] ->
    -- | Whether to include inline styles in the graph.
    Bool ->
    -- | The width dimension of the graph.
    Double ->
    -- | The height dimension of the graph.
    Double ->
    -- | The completed SVG document.
    S.Svg
    ...
    | otherwise =
        let style = case shapeType_ rect of
            ...
            class_ = case shapeType_ rect of
                ...
                _ -> ""
         in S.g
                ! A.id_ (textValue $ sanitizeId $ shapeId_ rect)
...
ellipseToSVG styled ellipse =
    S.g
        ! A.id_ (textValue (shapeId_ ellipse))
        ! A.transform (stringValue . formatTransform $ shapeTransform ellipse)
        ! A.class_ "bool"
        $ do
            S.ellipse
                ! A.cx (stringValue . show . fst $ shapePos ellipse)
  ...
  where
    ...
    allStyles =
        mconcat
            ...
            , A.fontSize (stringValue $ show fontSize ++ "pt")
            ]
            `mappend` baseStyles
...
edgeToSVG styled path =
    S.path
        ! A.id_ (textValue . T.append "path" . pathId_ $ path)
        ! A.class_ "path"
...
regionToSVG styled path =
    S.path
        ! A.id_ (textValue $ T.append "region" (pathId_ path))
        ! A.class_ "region"
        ! A.d (textValue . T.cons 'M' . buildPathString . pathPoints $ path)
        ! A.transform (stringValue . formatTransform $ pathTransform path)
        ! A.style
...
getTuple id_
  ...
  where
    tuples = M.filterWithKey (\k _ -> id_ `elem` k) areaMap
