module WebParsing.ArtSciParser (parseCalendar, parseDepartmentList) where
  ...
  where
    ...
    isValidDepartment ignoredDepartments (deptPage, deptName) =
        "/" `T.isPrefixOf` deptPage
            && deptName `notElem` ignoredDepartments -- Ignore footer links
            && not (" College)" `T.isSuffixOf` deptName) -- Ignore departments in ignoredDepartments
            -- Ignore departments belonging to a college
  ...
  where
    ...
    extractDepartments tableTags =
        ...
        let aTags = TS.partitions (tagOpenAttrNameLit "a" "href" (const True)) tableTags
         ...
         in filter (\(a, b) -> not (T.null a) && not (T.null b)) depts
      where
