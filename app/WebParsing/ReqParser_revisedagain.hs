...
parParser1 = do
    lpSeparator
    req <- Parsec.many (Parsec.noneOf [')'])
    return req

parParser2 :: String -> Req
parParser2 req =
    let tmp = parse parParser1 req
    in case tmp of
    Right parsed -> andorParser1 parsed
    Left _ -> J "fucked up fam"
