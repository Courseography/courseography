...
--drawJsonResponse :: String -> ServerPart Response
drawJsonResponse :: ServerPart Response
drawJsonResponse = do
   -- result <- liftIO $ getGraphJSON (read gId :: Int64)
   ok $ toResponse $
    masterTemplate "Courseography - Draw!"
                ...
                (do
                    ...
                    modePanel
                    -- loadJsonData result
                    -- loadJsonData gId -- \gId -> liftIO $ getGraphJSON (read gId :: Int64) >=
                )
                drawScripts

-- loadJsonData :: String -> H.Html
-- loadJsonData result = H.div ! A.id "json-data" $ H.toHtml $ show $ result
